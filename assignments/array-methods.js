...
let ticketPriceTotal = runners.reduce((x, y) => ({donation: x.donation + y.donation}));
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1: Each runner's e-mail needs to be put into a database. Create an array consolidating just the e-mails of each participant and log the result
let emails = runners.map(x => x.email);
console.log(emails);
...
let corporateBS = runners.map(x => `${x.first_name} ${x.last_name}, who works for ${x.company_name}, contributed \$${x.donation}.`);
console.log(corporateBS);